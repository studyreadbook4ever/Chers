...
class NativeVisionBridge:
    ...
    def observe(self, timeout=2.0):
        """Return ONLY a local observation ID and complete PNG image data URL."""
        with self._cv:
            self._check()
        ...
        with self._cv:
            while self._latest is None:
                ...
                if remaining <= 0:
                    raise TimeoutError("no public pixels received")
                self._cv.wait(remaining)
            self._check()
            receipt, rgba = self._latest
        png = rgba_png(rgba)
        with self._cv:
            self._check()
    ...
    def snapshot(self):
        with self._cv:
            ...
            return {"adapter": "native-full-frame-vision-explicit-keyboard-v1",
